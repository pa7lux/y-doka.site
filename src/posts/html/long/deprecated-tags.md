...
Веб-технологии не стоят на месте, появляются новые теги и CSS-свойства. Но некоторые теги «устаревают» и их использование уже не является хорошей практикой, а есть и такие, которые уже не поддерживаются браузером или никогда не были частью спецификации W3C.

Что делать, если в вашем проекте встретится один из таких тегов? Есть ли современная замена или альтернатива?

Попробуем разобраться.

Список всех «устаревших» тегов и свойств [на сайте WHATWG](https://html.spec.whatwg.org/multipage/obsolete.html).
...
```html
<acronym
  title="от англ. HyperText Markup Language — «язык гипертекстовой разметки»"
>HTML</acronym>
...
```html
<blink>Зачем кому-либо так делать? Выглядит антигуманно.</blink>
```

#### Чем заменить

Сложно придумать реальный кейс, когда подобное визуальное поведение не будет иметь явно отвлекающий эффект, но если «очень хочется», можно повторить подобное с помощью CSS:
...
Для стилизации текста. Да, до полноценной поддержки браузерами стилизации текста с помощью CSS это был единственный способ изменить цвет, шрифт или размер. К сожалению, для вёрстки писем под клиенты, не поддерживающие CSS, это остаётся единственной возможностью изменить внешний вид текста.
...
Хорошим тоном считается использование «склеивание» союзов и предлогов с последующим словом, единиц измерения.
...
Для остальных случаев есть соответсвующее CSS-свойство:
...
Достаточно популярный тег даже в наши дни, например, при вёрстке писем — почтовые клиенты сильно консервативны и порой для них это единственный способ центрирования.

#### Когда использовалось

Для выравнивания содержимого по центру «родителя».
...
```html
<div style="text-align:center">Мама, смотри — я в центре!</div>

<div style="width:500px; border:solid">
  <div style="margin:0 auto; width:400px; height:400px; border:solid">
    Этот вложенный блок отцентрирован по вертикали.
  </div>
</div>
```
...
Современных аналогов нет, можно сымитировать анимацию с помощью JavaScript и/или CSS.
